"""
Скрипт первичной загрузки 4-часовых свечей BTC в базу данных.
...
def main() -> None:
    """Основная функция загрузки 4-часовых свечей."""
    # 1. Инициализация БД
    ...
    # 2. Загрузка 4-часовых данных из Binance (с 2017 года)
    print("\n" + "=" * 60)
    print("📥 Шаг 1: Загрузка 4-часовых данных Binance")
    print("=" * 60)
    try:
        binance_fetcher = BinanceFetcher(timeframe="4h")
        binance_result = binance_fetcher.fetch(
            start_date=datetime(2017, 1, 1),
            end_date=datetime.now(),
        )
        if not binance_result.empty:
            TimeframeStorage(timeframe="4h").upsert_data(binance_result)
    ...
    # 3. Загрузка 4-часовых данных из Bybit (с 2020 года)
    print("\n" + "=" * 60)
    print("📥 Шаг 2: Загрузка 4-часовых данных Bybit")
    print("=" * 60)
    try:
        bybit_fetcher = BybitFetcher(timeframe="4h")
        bybit_result = bybit_fetcher.fetch(
            start_date=datetime(2020, 1, 1),
            end_date=datetime.now(),
        )
        if not bybit_result.empty:
            TimeframeStorage(timeframe="4h").upsert_data(bybit_result)
    ...
    # 4. Итог
    print("\n" + "=" * 60)
    print("✅ Загрузка 4-часовых свечей завершена!")
